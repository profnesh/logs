window.EasyLogin = {
	...
	alert: function (message, type, form) { 
		...
		if (typeof(message) == 'object') {
			...
			message = '<ul style="margin-bottom: 0px;">';
    ...
    $.amaran({
         'theme'     :'colorful',
        'content'   :{
           bgcolor: (type === 1 ? '#28a745' : type === 0 ? '#ffc107' : '#f40000'),
           color:'#fff',
           message: message
        },
        'stickyButton':true,
	...
	ajaxForm: function (e) {
		...
		var oldtext = btn.clone(); 
		btn.html(EasyLogin.trans('loadding'));
